...
class RestClient 
{
    ...
    public function __construct( $options = array() ){
        ...
        $default_options = array(
            ...
            'parameters' => array(), 
            'curl_options' => array(), 
            'user_agent' => "Fotkomotko RestClient/0.1", 
            'base_url' => NULL, 
            'format' => NULL, 
            'username' => NULL, 
            'password' => NULL,
    ...
    public function checkOptions($default_options) {
        ...
        if( $this->options['cache_enabled'] === true ) {
            ...
            if( !is_writable($this->options['cache_path']) ) {
                throw new Exception('Directory "'. $this->options['cache_path'] .'" (property: cache_path) is not writeable. Check permissions!');
            }
        }
    ...
    public function execute( $url, $method = 'GET', $parameters = array(), $headers = array() ){
        ...
        if( !empty($client->options['username']) && !empty($client->options['password']) ) {
            if( $client->options['auth_type'] === 'digest' ) {
